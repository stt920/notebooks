...
- 封装性：数据和代码捆绑在一起，避免外界干扰和不确定性访问。封装可以使得代码模块化。
  ...
  > 推崇封装的原因：它能使我们能够改变事物而只影响有些的客户（effective c++ pg：99）
...
- 派生类的功能可以被基类的方法或引用变量所调用，这叫向后兼容，可以提高**可扩充性和可维护性**。 //多态的真正作用，
...
nonstatic数据成员被放置到对象内部，static数据成员、static和nonstatic函数成员均被放到对象之外。对于虚函数的支持则分两部分完成：
